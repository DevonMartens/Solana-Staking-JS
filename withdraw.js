...
const main = async () => {
    ...
    const createStakeAccountTxn = StakeProgram.createAccount({
        ...
        const withdrawTx = StakeProgram.withdraw({
            stakePubkey: stakeAcct.publicKey,
            authorizedPubkey: wallet.publicKey,
            //wallet to place funds
            toPubkey: wallet.publicKey,
            //amount to withdraw
            lamports: stakingBalance,
        ...
        console.log("stake withdraw successful ",  `${withdrawTxID}`);
        let stakingBalance2 = await connection.getBalance(stakeAcct.publicKey);
        console.log("here's the balance of the stake account ",  `${stakingBalance2 / LAMPORTS_PER_SOL}`);
